...
import { estimatePrePurchaseQuote, estimateQuoteByServiceType, type PropertyDetails } from "@/lib/actions/quotes/estimateQuote";
...
export default async function StepQuote({ searchParams }: { searchParams?: Promise<Record<string, string | string[]>> }) {
	...
	if (!proposal && dealId) {
		...
		// Use service-specific quote estimation for pre-purchase
		const propertyDetails: PropertyDetails = {
			property_category,
			bedrooms: (property as any)?.number_of_bedrooms || 0,
			bathrooms: (property as any)?.number_of_bathrooms || 0,
		...
		try {
			const estimate = await estimatePrePurchaseQuote(propertyDetails);
			amount = estimate?.quote_price ?? 0;
			note = estimate?.note || undefined;
		} catch (error) {
			console.warn("[StepQuote] Pre-purchase quote estimation failed:", error);
		}
		...
		redirect(`/steps/04-quote/01-pre-purchase?${paramsOut.toString()}`);
	}
