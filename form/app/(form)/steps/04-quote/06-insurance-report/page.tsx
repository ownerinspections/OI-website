...
import { estimateInsuranceReportQuote, estimateQuoteByServiceType, type PropertyDetails } from "@/lib/actions/quotes/estimateQuote";
...
export default async function StepQuote({ searchParams }: { searchParams?: Promise<Record<string, string | string[]>> }) {
	...
	// Get initial estimated damage loss from property
	let initialEstimatedDamageLoss = 100000; // Default value
	let propertyCategory = "residential";
	if (propertyId) {
		try {
			const property = await getProperty(propertyId);
			initialEstimatedDamageLoss = (property as any)?.estimated_damage_loss || 100000;
			propertyCategory = (property as any)?.property_category || "residential";
		} catch {
	...
	if (quoteId && proposal && dealId) {
		try {
			...
			const selectedStageNumbers = dealInspectionStages.length > 0 
				...
				: ((property as any)?.stages || [1, 2, 3]); // Default 3 stages for insurance report
			...
			if (service?.service_type && property) {
				...
				try {
					...
					console.log("[StepQuote][InsuranceReport] All service stages:", allServiceStages);
				} catch (error) {
				...
				if (estimate && estimate.quote_price > 0) {
					...
					// Store stage prices for insurance report
					if (service.service_type === "insurance_report" && estimate.stage_prices) {
	...
	if (!proposal && dealId) {
		...
		try {
			const serviceRes = await getRequest<{ data: { stages?: any[] } }>(`/items/services/${encodeURIComponent(String(svcId))}?fields=stages`);
			allServiceStages = (serviceRes as any)?.data?.stages || [];
			console.log("[StepQuote][InsuranceReport][NewProposal] All service stages:", allServiceStages);
		} catch (error) {
			console.warn("Failed to fetch service stages:", error);
		}
		
		// Use service-specific quote estimation for insurance report
		const propertyDetails: PropertyDetails = {
			property_category,
			stages: allServiceStages.length > 0 
				? allServiceStages.map(s => Number(s.stage_number)).filter(n => Number.isFinite(n))
		...
		try {
			...
			console.warn("[StepQuote] Insurance report quote estimation failed:", error);
		}
		...
		redirect(`/steps/04-quote/06-insurance-report?${paramsOut.toString()}`);
	}
	...
	if (dealId) {
		try {
			...
			if (deal?.service) {
				...
				// Override service label to remove "Inspection" suffix for insurance reports
				const rawServiceLabel = service?.service_name || service?.service_type || undefined;
				serviceLabel = rawServiceLabel?.replace(/\s+Inspection\s*$/i, '') || undefined;
			}
			if (deal?.property) {
		...
		try {
			...
			if (addonIds.length > 0) {
				...
				if (Array.isArray((addonsRes as any)?.data) && (addonsRes as any).data.length > 0) {
					...
					console.warn("[StepQuote] Falling back to available addons for insurance report");
					
					// Fallback: Use available addons (1-9) for insurance report
	...
	return (
		<div className="container" data-quote-page>
			<div className="card">
				<FormHeader
					rightTitle="Quote"
					rightSubtitle={serviceLabel || "Insurance Report"}
					rightMeta={[
				...
				<QuotesForm 
					...
					showInspectionBox={true}
					estimatedDamageLoss={initialEstimatedDamageLoss}
