...
def _fetch_pricing_config() -> Dict[str, int]:
    ...
    Selects the row where service_type == 'pre_purchase'.
    """
    ...
    for it in items:
        try:
            if str(it.get("service_type")).strip().lower() == "pre_purchase":
                row = it
                break
        except Exception:
            continue
    if row is None:
        raise ValueError("Pricing for service 'pre_purchase' not found")
