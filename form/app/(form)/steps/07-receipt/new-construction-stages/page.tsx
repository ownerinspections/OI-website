...
export default async function ConstructionStagesReceiptStep({ searchParams }: { searchParams?: Promise<Record<string, string | string[]>> }) {
    ...
    if (paymentIntentIdParam || paymentIntentClientSecret) {
        ...
        if (invoiceId && isSucceeded && typeof paymentAmountAud === "number" && Number.isFinite(paymentAmountAud) && paymentAmountAud > 0) {
            try { console.log("[invoice][new-construction-stages-receipt-ssr-prg] Marking invoice paid", { invoiceId, amountPaid: paymentAmountAud }); } catch {}
            try {
        ...
        try {
            ...
            if (targetPaymentId) {
                const oiReceiptUrl = `${baseNoSlash}/steps/07-receipt/new-construction-stages?${sp.toString()}`;
                await patchRequest(`/items/os_payments/${encodeURIComponent(String(targetPaymentId))}`, { oi_receipt_url: oiReceiptUrl });
            }
        } catch {}
        redirect(`/steps/07-receipt/new-construction-stages?${sp.toString()}`);
    }
    ...
    try {
        if (paymentIdParam) {
            ...
            const oiReceiptUrl = `${baseNoSlash}/steps/07-receipt/new-construction-stages?${spSelf.toString()}`;
                ...
                try {
                    ...
                    if (!alreadyExists && invoice?.id) {
                        ...
                        const buildReceiptLink = (paymentIdArg?: string) => {
                            ...
                            return `${baseNoSlash}/steps/07-receipt/new-construction-stages?${sp.toString()}`;
                        };
                        ...
                        const createBody: Record<string, unknown> = {
                            ...
                            metadata: { source: "new-construction-stages-receipt-ssr" },
                        };
                        ...
                        try {
                            ...
                            try { console.warn('[new-construction-stages-receipt-ssr] failed to update deal stage to PAYMENT_SUBMITTED', err); } catch {}
                        }
    ...
    if (invoiceId && isPaid) {
        try { console.log("[invoice][new-construction-stages-receipt-ssr] Marking invoice paid (fallback)", { invoiceId, amountPaid: paidAmount }); } catch {}
        try {
