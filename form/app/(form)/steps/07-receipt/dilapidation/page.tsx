...
import { fetchCompanyInfo, fetchCustomerInfo, fetchPropertyInfo, fetchMultiplePropertiesInfo, CustomerInfo, CompanyInfo, PropertyInfo } from "@/lib/actions/invoices/createInvoice";
...
export default async function DilapidationReceiptStep({ searchParams }: { searchParams?: Promise<Record<string, string | string[]>> }) {
    ...
    if (paymentIntentIdParam || paymentIntentClientSecret) {
        ...
        if (invoiceId && isSucceeded && typeof paymentAmountAud === "number" && Number.isFinite(paymentAmountAud) && paymentAmountAud > 0) {
            try { console.log("[invoice][dilapidation-receipt-ssr-prg] Marking invoice paid", { invoiceId, amountPaid: paymentAmountAud }); } catch {}
            try {
        ...
        try {
            ...
            if (targetPaymentId) {
                const oiReceiptUrl = `${baseNoSlash}/steps/07-receipt/dilapidation?${sp.toString()}`;
                await patchRequest(`/items/os_payments/${encodeURIComponent(String(targetPaymentId))}`, { oi_receipt_url: oiReceiptUrl });
            }
        } catch {}
        redirect(`/steps/07-receipt/dilapidation?${sp.toString()}`);
    }
    ...
    try {
        if (paymentIdParam) {
            ...
            const oiReceiptUrl = `${baseNoSlash}/steps/07-receipt/dilapidation?${spSelf.toString()}`;
    ...
    const [companyInfo, customerInfo, propertiesInfo] = await Promise.allSettled([
        fetchCompanyInfo(),
    ...
    const properties = propertiesInfo.status === 'fulfilled' ? propertiesInfo.value : null;
                ...
                try {
                    ...
                    if (!alreadyExists && invoice?.id) {
                        ...
                        const buildReceiptLink = (paymentIdArg?: string) => {
                            ...
                            return `${baseNoSlash}/steps/07-receipt/dilapidation?${sp.toString()}`;
                        };
                        ...
                        const createBody: Record<string, unknown> = {
                            ...
                            metadata: { source: "dilapidation-receipt-ssr" },
                        };
                        ...
                        try {
                            ...
                            try { console.warn('[dilapidation-receipt-ssr] failed to update deal stage to PAYMENT_SUBMITTED', err); } catch {}
                        }
    ...
    if (invoiceId && isPaid) {
        try { console.log("[invoice][dilapidation-receipt-ssr] Marking invoice paid (fallback)", { invoiceId, amountPaid: paidAmount }); } catch {}
        try {
...
<div style={{ background: "var(--color-pale-gray)", padding: 16, borderRadius: 8 }}>
                ...
                {properties && properties.length > 0 ? (
                    properties.map((property, index) => (
                        <div key={index} style={{ background: "var(--color-pale-gray)", padding: 16, borderRadius: 8, marginBottom: 16 }}>
                            <h3 style={{ margin: "0 0 12px 0", color: "var(--color-primary)" }}>
                                {properties.length > 1 ? `Property ${index + 1} Details:` : 'Property Details:'}
                            </h3>
                            <div style={{ lineHeight: 1.5 }}>
                                {/* Full Address */}
