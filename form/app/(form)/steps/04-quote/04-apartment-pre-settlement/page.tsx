...
import { estimateApartmentPreSettlementQuote, estimateQuoteByServiceType, type PropertyDetails } from "@/lib/actions/quotes/estimateQuote";
...
export default async function StepQuote({ searchParams }: { searchParams?: Promise<Record<string, string | string[]>> }) {
	...
	if (!proposal && dealId) {
		...
		// Use service-specific quote estimation for apartment pre-settlement
		const propertyDetails: PropertyDetails = {
			property_category,
			bedrooms: (property as any)?.number_of_bedrooms || 0,
			bathrooms: (property as any)?.number_of_bathrooms || 0,
		...
		try {
			const estimate = await estimateApartmentPreSettlementQuote(propertyDetails);
			amount = estimate?.quote_price ?? 0;
			note = estimate?.note || undefined;
		} catch (error) {
			console.warn("[StepQuote] Apartment pre-settlement quote estimation failed:", error);
		}
		...
		redirect(`/steps/04-quote/04-apartment-pre-settlement?${paramsOut.toString()}`);
	}
	...
	return (
		<div className="container" data-quote-page>
			<div className="card">
				<FormHeader
					rightTitle="Quote"
					rightSubtitle={serviceLabel || "Apartment Pre-Settlement Inspection"}
					rightMeta={[
