...
export default async function ConstructionStagesInvoiceStep({ searchParams }: { searchParams?: Promise<Record<string, string | string[]>> }) {
		...
		if (dealId) {
			...
			if (deal?.service && propertyId && inspectionStages.length > 0) {
				try {
					...
					const { estimateConstructionStagesQuote } = await import("@/lib/actions/quotes/estimateQuote");
	...
	async function payNowAction() {
		...
		if (paymentId) {
			...
			try {
				...
				if ((existingRes as any)?.data) {
					...
					try { console.log("[os_payments][construction-stages-step5] Updating existing payment from URL", { paymentId: existingPaymentId, updateBody }); } catch {}
					await patchRequest(`/items/os_payments/${encodeURIComponent(String(paymentId))}`, updateBody);
					try { console.log("[os_payments][construction-stages-step5] Updated existing payment", { paymentId: existingPaymentId }); } catch {}
				}
			} catch {
		...
		if (!existingPaymentId) {
			try {
				...
				try { console.log("[os_payments][construction-stages-step5] Creating submitted payment", body); } catch {}
				const createdResp = await postRequest(`/items/os_payments`, body);
				try { console.log("[os_payments][construction-stages-step5] Created response", createdResp); } catch {}
				...
				if (createdPaymentId) {
					try {
						...
						try { console.log("[os_payments][construction-stages-step5] Updated payment with payment_link", { paymentId: createdPaymentId, payment_link: paymentPageUrl }); } catch {}
					} catch (err) {
						try { console.warn('[construction-stages-step5] failed to update payment with payment_link', err); } catch {}
					}
				}
				...
				try {
					...
					try { console.warn('[construction-stages-step5] failed to update deal stage to PAYMENT_SUBMITTED', err); } catch {}
				}
			} catch {}
		} else {
			// Use existing payment ID
			paymentIdFromCreation = existingPaymentId;
			try { console.log("[os_payments][construction-stages-step5] Using existing payment", { paymentId: existingPaymentId }); } catch {}
		}

		// Only change: set invoice status to approved
		try { console.log("[invoice][construction-stages-pay-now] Setting invoice status to approved", { invoiceId: currentInvoiceId }); } catch {}
		await updateInvoice(currentInvoiceId, { status: "approved" });
		try { console.log("[invoice][construction-stages-pay-now] Invoice status set to approved", { invoiceId: currentInvoiceId }); } catch {}
		...
		<div className="container">
			<div className="card">
				...
				<InvoicesForm invoice={{
					...
					return `/steps/04-quote/05-construction-stages?${prevParams.toString()}`;
