...
export async function submitBooking(prevState: BookingActionResult, formData: FormData): Promise<BookingActionResult> {
	...
	try {
		...
		console.log("🔍 [SUBMIT BOOKING] All form data entries:");
		...
		for (const ct of contactsList) {
			...
			console.log(`🔍 [SUBMIT BOOKING] Validating contact ${ct.id}:`, {
			...
			if (Object.keys(contactErrors).length > 0) {
				console.log(`❌ [SUBMIT BOOKING] Contact ${ct.id} validation errors:`, contactErrors);
			}
		...
		// Handle contact person for all inspections
		...
		if (inspectionType) {
			...
			console.log("🔍 [SUBMIT BOOKING] Found contact person choices:", contactPersonChoices);
			
			if (Object.keys(contactPersonChoices).length === 0) {
				console.log("⚠️ [SUBMIT BOOKING] No contact person choices found in form data");
				// Don't set a fallback - let the system handle this case
				console.log("⚠️ [SUBMIT BOOKING] No contact person will be set for this deal");
			}
			...
			for (const [fieldName, choice] of Object.entries(contactPersonChoices)) {
				...
				if (choice === "new_contact") {
					...
					console.log(`🔍 [SUBMIT BOOKING] Contact person data for property ${propertyId}:`, contactPersonData);
					...
					if (contactPersonData.first_name && contactPersonData.last_name && (contactPersonData.email || contactPersonData.contact_type === "real_estate_agent")) {
						console.log(`🆕 [SUBMIT BOOKING] Creating contact person for property ${propertyId}`);
						try {
							...
							console.log(`🔍 [SUBMIT BOOKING] Contact person input:`, contactPersonInput);
							
							const res = await createContact(contactPersonInput);
							console.log(`🔍 [SUBMIT BOOKING] Create contact response:`, res);
							
							if (res?.success && res?.contactId) {
								contactPersonId = res.contactId;
								console.log(`✅ [SUBMIT BOOKING] Created contact person with ID: ${contactPersonId}`);
								
								// Link contact person to current user
								if (userId) {
									try {
										await updateContact(contactPersonId, { user: userId } as any);
										console.log(`✅ [SUBMIT BOOKING] Successfully linked contact person ${contactPersonId} to user ${userId}`);
									} catch (linkError) {
										console.error(`❌ [SUBMIT BOOKING] Failed to link contact person ${contactPersonId} to user ${userId}:`, linkError);
									}
								}
								...
								if (dealId) {
									try {
										...
										console.log(`✅ [SUBMIT BOOKING] Successfully linked contact person ${contactPersonId} to deal ${dealId} via os_deals_contacts`);
									} catch (junctionError) {
										console.error(`❌ [SUBMIT BOOKING] Failed to link contact person ${contactPersonId} to deal ${dealId} via os_deals_contacts:`, junctionError);
									}
								}
							} else {
								console.error(`❌ [SUBMIT BOOKING] Failed to create contact person - invalid response:`, res);
							}
						} catch (error) {
							console.error(`❌ [SUBMIT BOOKING] Failed to create contact person for property ${propertyId}:`, error);
						}
					} else {
						console.log(`⚠️ [SUBMIT BOOKING] Missing required fields for contact person - first_name: ${contactPersonData.first_name}, last_name: ${contactPersonData.last_name}, email: ${contactPersonData.email}`);
					}
				} else if (choice === "me") {
					// Use the current contact as contact person
					contactPersonId = contactId;
					console.log(`✅ [SUBMIT BOOKING] Using current contact ${contactId} as contact person for property ${propertyId}`);
				}
				
				// For now, we'll use the last processed contact person
				// In a multi-property scenario, you might want to handle this differently
				break;
			}
			...
			if (Object.keys(contactPersonChoices).length === 0 && contactId) {
				contactPersonId = contactId;
				console.log(`✅ [SUBMIT BOOKING] No contact person choices found, defaulting to main contact ${contactId} as contact person`);
			}
		}
						...
						if (res?.success && res?.contactId) {
							...
							if (dealId) {
								try {
									...
									console.log(`✅ [SUBMIT BOOKING] Successfully linked contact ${res.contactId} to deal ${dealId} via os_deals_contacts`);
								} catch (junctionError) {
									console.error(`❌ [SUBMIT BOOKING] Failed to link contact ${res.contactId} to deal ${dealId} via os_deals_contacts:`, junctionError);
								}
							} else {
								console.warn(`⚠️ [SUBMIT BOOKING] No dealId provided, contact ${res.contactId} not linked to deal`);
							}
						}
					}
				}
		...
		if (booking?.id != null) {
			...
			if (contactPersonId) {
				...
				if (!allContactIds.some(id => String(id) === contactPersonIdStr)) {
					...
					console.log(`📝 [SUBMIT BOOKING] Adding contact_person ${contactPersonId} to booking contacts array`);
				}
			}
			...
			if (allContactIds.length > 0) {
				...
				console.log(`📝 [SUBMIT BOOKING] Setting booking contacts array with ${allContactIds.length} contacts:`, allContactIds);
			}
		...
		if (dealId) {
			...
			if (inspectionType === "dilapidation") {
				console.log(`🔍 [SUBMIT BOOKING] Processing dilapidation contact person updates for deal ${dealId}`);
				...
				try {
					...
					console.log(`🔍 [SUBMIT BOOKING] Properties in order for deal ${dealId}:`, propertiesInOrder);
				} catch (error) {
					console.error(`❌ [SUBMIT BOOKING] Failed to load properties for deal ${dealId}:`, error);
				}
				...
				for (const [fieldName, choice] of Object.entries(contactPersonChoices)) {
					...
					if (propertyIndex === -1) {
						console.warn(`⚠️ [SUBMIT BOOKING] Property ${propertyId} not found in deal properties list`);
						continue;
					}
								...
								if (res?.success && res?.contactId) {
									...
									if (userId) {
										try {
											...
											console.error(`❌ [SUBMIT BOOKING] Failed to link contact person ${propertyContactPersonId} to user ${userId}:`, linkError);
										}
									}
									...
									try {
										...
										console.error(`❌ [SUBMIT BOOKING] Failed to link contact person ${propertyContactPersonId} to deal ${dealId} via os_deals_contacts:`, junctionError);
									}
								}
							} catch (error) {
								console.error(`❌ [SUBMIT BOOKING] Failed to create contact person for property ${propertyId}:`, error);
					...
					if (propertyContactPersonId) {
						...
						console.log(`🔗 [SUBMIT BOOKING] Updating deal ${dealId} with ${contactPersonField} for property ${propertyId}: ${propertyContactPersonId}`);
					} else {
						console.warn(`⚠️ [SUBMIT BOOKING] No contact person ID determined for property ${propertyId} with choice "${choice}"`);
					}
				}
			} else {
				...
				if (contactPersonId) {
					...
					console.log(`🔗 [SUBMIT BOOKING] Updating deal ${dealId} with contact_person: ${contactPersonId}`);
				} else {
					console.warn(`⚠️ [SUBMIT BOOKING] No contact person determined for deal ${dealId} - this may indicate an issue with the contact person selection logic`);
				}
			}
			...
			try {
				...
				if (dealValue && dealValue > 0) {
					...
					console.log(`💰 [SUBMIT BOOKING] Deal ${dealId} already has deal_value: ${dealValue}`);
				} else {
					console.warn(`⚠️ [SUBMIT BOOKING] Deal ${dealId} has no deal_value set - this may indicate an issue with the quote step`);
				}
			} catch (error) {
				console.error(`❌ [SUBMIT BOOKING] Failed to load deal_value from deal ${dealId}:`, error);
			}
			
			// Apply updates if any
			if (Object.keys(dealUpdateData).length > 0) {
				console.log(`🔧 [SUBMIT BOOKING] About to update deal ${dealId} with data:`, dealUpdateData);
				try {
					await patchRequest(`/items/os_deals/${encodeURIComponent(String(dealId))}`, dealUpdateData);
					console.log(`✅ [SUBMIT BOOKING] Successfully updated deal ${dealId} with:`, dealUpdateData);
				} catch (error) {
					console.error(`❌ [SUBMIT BOOKING] Failed to update deal ${dealId}:`, error);
				}
			} else {
				console.warn(`⚠️ [SUBMIT BOOKING] No deal update data to apply for deal ${dealId}`);
			}
		}
