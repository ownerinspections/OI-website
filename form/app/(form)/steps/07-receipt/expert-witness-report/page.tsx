...
export default async function ExpertWitnessReportReceiptStep({ searchParams }: { searchParams?: Promise<Record<string, string | string[]>> }) {
    ...
    if (paymentIntentIdParam || paymentIntentClientSecret) {
        ...
        if (invoiceId && isSucceeded && typeof paymentAmountAud === "number" && Number.isFinite(paymentAmountAud) && paymentAmountAud > 0) {
            try { console.log("[invoice][expert-witness-report-receipt-ssr-prg] Marking invoice paid", { invoiceId, amountPaid: paymentAmountAud }); } catch {}
            try {
        ...
        try {
            ...
            if (targetPaymentId) {
                const oiReceiptUrl = `${baseNoSlash}/steps/07-receipt/expert-witness-report?${sp.toString()}`;
                await patchRequest(`/items/os_payments/${encodeURIComponent(String(targetPaymentId))}`, { oi_receipt_url: oiReceiptUrl });
            }
        } catch {}
        redirect(`/steps/07-receipt/expert-witness-report?${sp.toString()}`);
    }
    ...
    try {
        if (paymentIdParam) {
            ...
            const oiReceiptUrl = `${baseNoSlash}/steps/07-receipt/expert-witness-report?${spSelf.toString()}`;
                ...
                try {
                    ...
                    if (!alreadyExists && invoice?.id) {
                        ...
                        const buildReceiptLink = (paymentIdArg?: string) => {
                            ...
                            return `${baseNoSlash}/steps/07-receipt/expert-witness-report?${sp.toString()}`;
                        };
                        ...
                        const createBody: Record<string, unknown> = {
                            ...
                            metadata: { source: "expert-witness-report-receipt-ssr" },
                        };
                        ...
                        try {
                            ...
                            try { console.warn('[expert-witness-report-receipt-ssr] failed to update deal stage to PAYMENT_SUBMITTED', err); } catch {}
                        }
    ...
    if (invoiceId && isPaid) {
        try { console.log("[invoice][expert-witness-report-receipt-ssr] Marking invoice paid (fallback)", { invoiceId, amountPaid: paidAmount }); } catch {}
        try {
            ...
            <div className="card">
                ...
                <div className="button-container" style={{ display: "flex", justifyContent: "flex-end", marginTop: 16 }}>
                    ...
                    <form action={async () => {
                        ...
                        redirect(`/steps/08-booking/expert-witness-report?${sp.toString()}`);
                    }}>
