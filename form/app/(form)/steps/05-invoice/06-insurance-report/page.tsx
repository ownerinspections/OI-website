...
export default async function InsuranceReportInvoiceStep({ searchParams }: { searchParams?: Promise<Record<string, string | string[]>> }) {
		...
		if (dealId) {
			...
			if (deal?.service && propertyId && inspectionStages.length > 0) {
				try {
					...
					const { estimateInsuranceReportQuote } = await import("@/lib/actions/quotes/estimateQuote");
					...
					if (property) {
						const propertyDetails = {
							...
							estimated_damage_loss: (property as any)?.estimated_damage_loss || 100000,
						};
			...
			if (displayLineItems.length === 0) {
				let serviceName: string = "Insurance Report Inspection";
				if (deal?.service) {
					try {
	...
	async function payNowAction() {
		...
		if (paymentId) {
			...
			try {
				...
				if ((existingRes as any)?.data) {
					...
					try { console.log("[os_payments][insurance-report-step5] Updating existing payment from URL", { paymentId: existingPaymentId, updateBody }); } catch {}
					await patchRequest(`/items/os_payments/${encodeURIComponent(String(paymentId))}`, updateBody);
					try { console.log("[os_payments][insurance-report-step5] Updated existing payment", { paymentId: existingPaymentId }); } catch {}
				}
			} catch {
		...
		if (!existingPaymentId) {
			try {
				...
				try { console.log("[os_payments][insurance-report-step5] Creating submitted payment", body); } catch {}
				const createdResp = await postRequest(`/items/os_payments`, body);
				try { console.log("[os_payments][insurance-report-step5] Created response", createdResp); } catch {}
				...
				if (createdPaymentId) {
					try {
						...
						try { console.log("[os_payments][insurance-report-step5] Updated payment with payment_link", { paymentId: createdPaymentId, payment_link: paymentPageUrl }); } catch {}
					} catch (err) {
						try { console.warn('[insurance-report-step5] failed to update payment with payment_link', err); } catch {}
					}
				}
				...
				try {
					...
					try { console.warn('[insurance-report-step5] failed to update deal stage to PAYMENT_SUBMITTED', err); } catch {}
				}
			} catch {}
		} else {
			// Use existing payment ID
			paymentIdFromCreation = existingPaymentId;
			try { console.log("[os_payments][insurance-report-step5] Using existing payment", { paymentId: existingPaymentId }); } catch {}
		}

		// Only change: set invoice status to approved
		try { console.log("[invoice][insurance-report-pay-now] Setting invoice status to approved", { invoiceId: currentInvoiceId }); } catch {}
		await updateInvoice(currentInvoiceId, { status: "approved" });
		try { console.log("[invoice][insurance-report-pay-now] Invoice status set to approved", { invoiceId: currentInvoiceId }); } catch {}
		...
		<div className="container">
			<div className="card">
				...
				<InvoicesForm invoice={{
					...
					return `/steps/04-quote/06-insurance-report?${prevParams.toString()}`;
