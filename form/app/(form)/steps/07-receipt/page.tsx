...
export default async function ReceiptStep({ searchParams }: { searchParams?: Promise<Record<string, string | string[]>> }) {
	...
	const receiptFolder = oldFolderMapping[serviceRoute] || serviceRoute;
	
	// Build the URL parameters for the service-specific receipt page
	...
	if (invoiceId) receiptParams.set("invoiceId", String(invoiceId));
	if (paymentId) receiptParams.set("paymentId", String(paymentId));
	if (paymentIntent) receiptParams.set("payment_intent", String(paymentIntent));
	if (paymentIntentClientSecret) receiptParams.set("payment_intent_client_secret", String(paymentIntentClientSecret));

	// Redirect to the service-specific receipt page
	const targetUrl = `/steps/07-receipt/${receiptFolder}?${receiptParams.toString()}`;
	redirect(targetUrl);
}
