...
import { estimateExpertWitnessReportQuote, estimateQuoteByServiceType, type PropertyDetails } from "@/lib/actions/quotes/estimateQuote";
...
export default async function StepQuote({ searchParams }: { searchParams?: Promise<Record<string, string | string[]>> }) {
	...
	if (!proposal && dealId) {
		...
		redirect(`/steps/04-quote/07-expert-witness-report?${paramsOut.toString()}`);
	}
	...
	if (dealId) {
		try {
			...
			if (deal?.service) {
				...
				// Override service label to remove "Inspection" suffix for expert witness reports
		...
		try {
			...
			if (addonIds.length > 0) {
				...
				if (Array.isArray((addonsRes as any)?.data) && (addonsRes as any).data.length > 0) {
					...
					console.warn("[StepQuote] Falling back to available addons for expert witness report");
					
					// Fallback: Use available addons (1-9) for expert witness report
	...
	return (
		<div className="container" data-quote-page>
			<div className="card">
				<FormHeader
					rightTitle="Quote"
					rightSubtitle={serviceLabel || "Expert Witness Report"}
					rightMeta={[
